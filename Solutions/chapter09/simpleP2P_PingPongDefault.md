...
inline bool isUnifiedAddressingSupported(int ngpus)
{
    ...
    for (int i = 0; i < ngpus; i++) CHECK(cudaGetDeviceProperties(&prop[i], i));

    const bool iuva = (prop[0].unifiedAddressing && prop[1].unifiedAddressing);
    printf("> GPU0: %s %s unified addressing\n", prop[0].name,
           (prop[0].unifiedAddressing ? "supports" : "does not support"));
    printf("> GPU1: %s %s unified addressing\n", prop[1].name,
           (prop[1].unifiedAddressing ? "supports" : "does not support"));
    return iuva;
}
...
int main(int argc, char **argv)
{
    ...
    elapsed_time_ms /= 100.0f;
    printf("Ping-pong unidirectional cudaMemcpyAsync:\t %8.2fms ",
           elapsed_time_ms);
    printf("performance: %8.2f GB/s\n",
            (float)iBytes / (elapsed_time_ms * 1e6f) );
    ...
    for (int i = 0; i < 100; i++)
    {
        CHECK(cudaMemcpyPeerAsync(d_src[1], 1, d_src[0], 0, iBytes));
        CHECK(cudaMemcpyPeerAsync(d_rcv[0], 0, d_rcv[1], 1, iBytes));
    }
    ...
    elapsed_time_ms /= 100.0f;
    printf("Ping-pong bidirectional cudaMemcpyPeerAsync:\t %8.2fms ",
           elapsed_time_ms);
    printf("performance: %8.2f GB/s\n",
           (float) 2.0f * iBytes / (elapsed_time_ms * 1e6f) );
    ...
    printf("\nRunning kernel on GPU1, taking source data from GPU0 and "
            "writing to GPU1...\n");
    ...
    printf("Running kernel on GPU0, taking source data from GPU1 and "
            "writing to GPU0...\n");
    CHECK(cudaSetDevice(0));
